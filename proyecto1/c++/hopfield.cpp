...
void weigths(int **W, int *aries, int *tauro, int *geminis, int *cancer,int*leo,int*virgo, int N)
{

  for (int i=0; i<N; i++)
  for (int j=0; j<N; j++)
  {
    W[i][j] = aries[i]*aries[j] + tauro[i]*tauro[j] + geminis[i]*geminis[j] + cancer[i]*cancer[j] + leo[i]*leo[j] + virgo[i]*virgo[j] ;
  }
  for (int k=0; k<N; k++)
    W[k][k]=0;
}
...
int main(void)
{
  ...
  int *aries = new int[N];
  int *tauro = new int[N];
  int *geminis = new int[N];
  int *cancer = new int[N];
  int *leo = new int[N];
  int *virgo = new int[N];

  cout << "iterations " << endl;

  readfile("1.txt", aries,  col, N);
  readfile("2.txt", tauro,  col, N);
  readfile("3.txt", geminis,  col, N);
  readfile("4.txt", cancer,  col, N);
  readfile("5.txt", leo,  col, N);
  readfile("6.txt", virgo,  col, N);
...
*/
 ...
 W[i] = new int[N];
  ...
  weigths(W, aries, tauro, geminis, cancer, leo, virgo, N);
  printw(W, N);
